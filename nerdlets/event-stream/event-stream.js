  ...
  startTimer() {
    this.refresh = setInterval(async () => {
      if (this.state.enabled && this.state.queryStatus != 'start') {
        ...
        let baseQuery = `SELECT ${setQueryAttributes(
          columns
        )} FROM Transaction, TransactionError WHERE entityGuid = '${entityGuid}'`;
        ...
        // construct query
        let query = `${baseQuery} `;
        query += Object.keys(this.state.filters)
          .map(filter => `AND ${this.state.filters[filter]} `)
          .toString()
          .replace(/,/g, '');
        ...
        query += ` AND traceId NOT IN (${"'" +
          previousIds.join("','") +
          "'"}) `;

        // fetch events
        let nerdGraphResults = await NerdGraphQuery.query({
          query: gql`
            ${eventStreamQuery(
              entityGuid,
              entity.account.id,
        ...
        let nerdGraphData =
          (((nerdGraphResults || {}).data || {}).actor || {}).account || {};
        ...
        let filterIds = result
          .filter(event => event.timestamp == queryTimestamp)
    ...
    if (entityGuid) {
      ...
      if (data.actor.entity) {
        ...
        switch (data.actor.entity.domain) {
          case 'APM':
            baseQuery = `SELECT ${setQueryAttributes(
              columns
            )} FROM Transaction, TransactionError WHERE entityGuid = '${entityGuid}'`;
            console.log(`NRDB Base Query: ${baseQuery}`);
            keySet = await nrdbQuery(
              data.actor.entity.account.id,
              `SELECT keyset() FROM Transaction, TransactionError WHERE entityGuid = '${entityGuid}'`
            );
        ...
        await this.setState({
          ...
          accountId: data.actor.entity.account.id,
          keySet,
        });
        this.startTimer();
      } else {
        ...
        console.log(
          'entity is null, ensure this has been deployed against the correct account'
        );
      }
    } else {
      await this.setState({ entity: null });
    ...
    return (
      <Grid style={{ height: '100%' }}>
        ...
        <Grid.Row
          columns={16}
          stretched
          style={{ height: '77%', paddingTop: '0px' }}
        >
          <Grid.Column width={16}>
            <Dimmer
              active={dimmerOn}
              style={{ display: dimmerOn ? '' : 'none', height: '108%' }}
            >
              <Loader
                size="large"
                style={{
                  display:
                    (loading || eventLength.length == 0) &&
                    !unableToAccessEntity
                      ? ''
                      : 'none',
                }}
              >
              ...
              <Header
                inverted
                style={{ display: unableToAccessEntity ? '' : 'none' }}
              >
                Nerdpack unable to access this entity!
                <Header.Subheader>
                  Have you deployed against the correct account?
                </Header.Subheader>
              </Header>
            </Dimmer>
            <h3
            ...
            >
              {stats.count || 0} Transactions since 1 minute ago
            </h3>
            <EventTable
              style={{ display: dimmerOn ? 'none' : '' }}
              events={events}
              columns={columns}
              query={queryTracker}
